...
#define S_CMD_MAP                                                                                   \
    ((1 << S_CMD_NOP) | (1 << S_CMD_Q_IFACE) | (1 << S_CMD_Q_CMDMAP) | (1 << S_CMD_Q_PGMNAME) |     \
     (1 << S_CMD_Q_SERBUF) | (1 << S_CMD_Q_BUSTYPE) | (1 << S_CMD_SYNCNOP) | (1 << S_CMD_O_SPIOP) | \
     (1 << S_CMD_S_BUSTYPE) | (1 << S_CMD_S_SPI_FREQ) | (1 << S_CMD_S_PIN_STATE))
...
void process(pio_spi_inst_t *spi, int command) {
    switch (command) {
        case S_CMD_NOP: putchar(S_ACK); break;
...
int main() {
    ...
    // We use PIO 1
    pio_spi_inst_t spi = {.pio = pio1, .sm = 0, .cs_pin = PIN_CS};
